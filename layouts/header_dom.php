<div class="header bg-dark" style="background-image: url('https://sun7-6.userapi.com/s/v1/d/N8aALwnmnh8IP0dChqRjyR-B0id0I4WTkyhJQ2QFd7SxMUSeqKel-v8-5bfoF8k72F6iTErEeepXtyWIV6hcsBgLxy-YSxOkKxxljeBLoP4XdGfDF6Iehw/oblozhka.png')  ">
  
  <?php 
  include 'header.php';
  ?>
 
  <?php
  include 'menu_desctop.php'; 
  ?>

  <!-- мобильное -->
  <div class="container mt-2 pb-2 d-block d-sm-none">
	<div class="row">
		<div class="col text-light">
			<div type="" class="text-light border-0 bg-none">
				<div class="row g-2" data-bs-toggle="dropdown" href="#" role="button" aria-expanded="false">
					<div class="col-auto">
						<a class="h2 text-light" style="text-decoration: none">
							МЕНЮ
						</a>
					</div>
					<div class="col-auto">
						<img class="pt-1" style="max-width: 25px; transform: rotate (180deg) !important;" src="ant-design_lerft-outl1.svg" alt="" />
					</div>
				</div>

				<ul class="dropdown-menu text-start position-fixed" style="background: none ; height:100vh;  margin-left:0; margin-right:0; position:absolute; margin-top:100px !important;">
					<div class="modal-dialog modal-fullscreen" style="background: none ; height:100vh; margin-top: -175px !important; margin-left:0; margin-right:0;">
						<div class="modal-content bg-dark text-light" style=" margin-left:0; margin-right:0;">
							<div class="modal-header border-0">
								<h5 class="modal-title" id="exampleModalLabel">
									<img class="" src="fonts/ant-design_home-outl.svg" alt="" />
								</h5>
								<button type="button" class="btn-close text-light btn-close-white p-4" data-bs-dismiss="modal" aria-label="Close"></button>
							</div>
							<div class="modal-body">
								<div class="row gy-3">
									<div class="col-12">
										<span class="h5" data-bs-dismiss="modal" aria-label="Close"><a href="../../#rec448333531" style="text-decoration: none !important; color: #fff;">ПРОЕКТЫ ДОМОВ</a></span>
									</div>
									<div class="col-12">
										<span class="h5" data-bs-dismiss="modal" aria-label="Close">СТРОЙМАТЕРИАЛЫ</span>
										<li class="nav-link text-light" data-bs-dismiss="modal" aria-label="Close">
											<a class="border-bottom border-1 border-light text-light text-decoration-none" onclick="modalSow('Интересуют пенобетонные блоки?');">Пенобетонные блоки</a>
										</li>
										<li class="nav-link text-light" data-bs-dismiss="modal" aria-label="Close">
											<a class="border-bottom border-1 border-light text-light text-decoration-none" onclick="modalSow('Интересуют полистеролбетонные блоки?');">Полистеролбетонные блоки</a>
										</li>
									</div>
									<div class="col-12">
										<a href="https://simprolitstroy.ru/" data-bs-dismiss="modal" aria-label="Close" style="text-decoration: none !important; color: #fff;"><span class="h5">ДОКУМЕНТАЦИЯ</span></a>
									</div>
									<div class="col-12">
										<a href="#rec448407412" style="text-decoration: none !important; color: #fff;"><span class="h5">ОТЗЫВЫ</span></a>
									</div>
									<div class="col-12">
										<a href="#rec448425078" style="text-decoration: none !important; color: #fff;"><span class="h5">КОНТАКТЫ</span></a>
									</div>
								</div>
							</div>

							<div class="modal-footer border-0 display-flex justify-content-start w-100 margin-0">
								<a href="https://vk.me/public212435437">
									<svg
										version="1.1"
										id="Layer_1"
										xmlns="http://www.w3.org/2000/svg"
										xlink="http://www.w3.org/1999/xlink"
										x="0px"
										y="0px"
										width="30px"
										height="30px"
										viewBox="0 0 99 99"
										enable-background="new 0 0 99 99"
										space="preserve"
									>
										<path
											style="fill:#ffffff;"
											fill-rule="evenodd"
											clip-rule="evenodd"
											d="M49 98C76.062 98 98 76.062 98 49C98 21.938 76.062 0 49 0C21.938 0 0 21.938 0 49C0 76.062 21.938 98 49 98ZM22 34C22.4442 55.3198 33.1042 68.1323 51.7933 68.1323H52.8527V55.9351C59.7202 56.6182 64.9132 61.6409 66.9973 68.1323H76.7008C74.0359 58.4292 67.0314 53.0649 62.658 51.0149C67.0314 48.4866 73.1814 42.3367 74.6505 34H65.8353C63.922 40.7649 58.251 46.915 52.8527 47.4958V34H44.0374V57.6433C38.5707 56.2766 31.6692 49.6482 31.3617 34H22Z"
										></path>
									</svg>
								</a>
								<a href="https://t.me/simprolitstoy_bot">
									<svg class="t-sociallinks__svg" version="1.1" id="Layer_1" xmlns="http://www.w3.org/2000/svg" xlink="http://www.w3.org/1999/xlink" width="30px" height="30px" viewBox="0 0 60 60" space="preserve">
										<desc>Telegram</desc>
										<path
											style="fill:#ffffff;"
											d="M30 0C13.4 0 0 13.4 0 30s13.4 30 30 30 30-13.4 30-30S46.6 0 30 0zm16.9 13.9l-6.7 31.5c-.1.6-.8.9-1.4.6l-10.3-6.9-5.5 5.2c-.5.4-1.2.2-1.4-.4L18 32.7l-9.5-3.9c-.7-.3-.7-1.5 0-1.8l37.1-14.1c.7-.2 1.4.3 1.3 1z"
										></path>
										<path style="fill:#ffffff;" d="M22.7 40.6l.6-5.8 16.8-16.3-20.2 13.3"></path>
									</svg>
								</a>
								<a href="whatsapp://send?phone=78432126177">
									<svg
										version="1.1"
										id="Layer_1"
										xmlns="http://www.w3.org/2000/svg"
										xlink="http://www.w3.org/1999/xlink"
										x="0px"
										y="0px"
										width="30px"
										height="30px"
										viewBox="0 0 99 99"
										style="enable-background:new 0 0 99 99;"
										space="preserve"
									>
										<path
											style="fill:#ffffff;"
											d="M63.892 53.114C63.143 52.739 59.462 50.928 58.775 50.678C58.089 50.428 57.589 50.303 57.09 51.053C56.591 51.803 55.156 53.489 54.719 53.989C54.282 54.489 53.845 54.551 53.097 54.176C52.348 53.801 49.935 53.011 47.075 50.459C44.849 48.473 43.346 46.021 42.909 45.272C42.472 44.522 42.862 44.117 43.237 43.744C43.574 43.408 43.986 42.869 44.36 42.432C44.734 41.995 44.859 41.682 45.109 41.183C45.359 40.683 45.234 40.246 45.047 39.871C44.86 39.496 43.362 35.81 42.738 34.311C42.13 32.851 41.513 33.049 41.053 33.026C40.617 33.004 40.117 33 39.618 33C39.119 33 38.308 33.187 37.621 33.937C36.935 34.687 35 36.498 35 40.184C35 43.87 37.683 47.431 38.058 47.93C38.432 48.43 43.338 55.993 50.85 59.237C52.637 60.009 54.031 60.469 55.119 60.814C56.913 61.384 58.545 61.304 59.836 61.111C61.275 60.896 64.266 59.3 64.89 57.551C65.514 55.802 65.514 54.302 65.327 53.99C65.14 53.677 64.641 53.489 63.892 53.114Z"
										></path>
										<path
											style="fill:#ffffff;"
											fill-rule="evenodd"
											clip-rule="evenodd"
											d="M49 98C76.062 98 98 76.062 98 49C98 21.938 76.062 0 49 0C21.938 0 0 21.938 0 49C0 76.062 21.938 98 49 98ZM50.221 17C58.238 17.003 65.763 20.124 71.421 25.789C77.08 31.454 80.194 38.983 80.189 46.992C80.182 63.515 66.737 76.96 50.22 76.96H50.208C45.193 76.958 40.264 75.7 35.887 73.312L20 77.479L24.252 61.949C21.63 57.405 20.25 52.249 20.252 46.967C20.258 30.443 33.703 17 50.221 17Z"
										></path>
									</svg>
								</a>
							</div>
						</div>
					</div>
				</ul>
			</div>
		</div>
		<div class="col text-light d-flex justify-content-end">
			<div type="button" class="text-light border-0 bg-none">
				<div class="row g-2" data-bs-toggle="dropdown" href="#" role="button" aria-expanded="false">
					<div class="col-auto">
						<img class="pt-1" style="max-width: 25px; transform: rotate (180deg) !important;" src="ant-design_left-outl.svg" alt="" />
					</div>
					<div class="col-auto">
						<a class="h2 text-light" style="text-decoration: none">
							УСЛУГИ
						</a>
					</div>
				</div>

				<ul class="dropdown-menu text-start position-fixed" style="background: none ; height:100vh;  margin-left:0; margin-right:0; position:absolute; margin-top:100px !important;">
					<div class="modal-dialog modal-fullscreen" style="background: none ; height:100vh; margin-top: -175px !important; margin-left:0; margin-right:0;">
						<div class="modal-content bg-dark text-light" style=" margin-left:0; margin-right:0;">
							<div class="modal-header border-0">
								<h5 class="modal-title" id="exampleModalLabel">
									<img class="" src="fonts/ant-design_home-outl.svg" alt="" />
								</h5>
								<button type="button" class="btn-close text-light btn-close-white p-4" data-bs-dismiss="modal" aria-label="Close"></button>
							</div>
							<div class="modal-body" id="navbar-example3">
								<div class="row gy-3">
									<div class="col-12">
										<a href="../../" style="text-decoration: none !important; color: #fff;" data-bs-dismiss="modal" aria-label="Close"><span class="h5">Строительство под ключ</span></a>
									</div>
									<div class="col-12">
										<a href="https://sfsdf.test/#scrollspyHeading1" class="" style="text-decoration: none !important; color: #fff;"><span class="h5">Утепление крыш</span></a>
									</div>
									<div class="col-12">
										<span class="h5" data-bs-dismiss="modal" aria-label="Close" onclick="modalSow('Интересует утепление полов?');">Утепление полов</span>
									</div>
									<div class="col-12">
										<span class="h5" data-bs-dismiss="modal" aria-label="Close" onclick="modalSow('Интересует утепление стен?');">Утепление стен</span>
									</div>
								</div>
							</div>
							<div class="modal-footer border-0 display-flex justify-content-start w-100 margin-0">
								<a href="https://vk.me/public212435437">
									<svg
										version="1.1"
										id="Layer_1"
										xmlns="http://www.w3.org/2000/svg"
										xlink="http://www.w3.org/1999/xlink"
										x="0px"
										y="0px"
										width="30px"
										height="30px"
										viewBox="0 0 99 99"
										enable-background="new 0 0 99 99"
										space="preserve"
									>
										<path
											style="fill:#ffffff;"
											fill-rule="evenodd"
											clip-rule="evenodd"
											d="M49 98C76.062 98 98 76.062 98 49C98 21.938 76.062 0 49 0C21.938 0 0 21.938 0 49C0 76.062 21.938 98 49 98ZM22 34C22.4442 55.3198 33.1042 68.1323 51.7933 68.1323H52.8527V55.9351C59.7202 56.6182 64.9132 61.6409 66.9973 68.1323H76.7008C74.0359 58.4292 67.0314 53.0649 62.658 51.0149C67.0314 48.4866 73.1814 42.3367 74.6505 34H65.8353C63.922 40.7649 58.251 46.915 52.8527 47.4958V34H44.0374V57.6433C38.5707 56.2766 31.6692 49.6482 31.3617 34H22Z"
										></path>
									</svg>
								</a>
								<a href="https://t.me/simprolitstoy_bot">
									<svg class="t-sociallinks__svg" version="1.1" id="Layer_1" xmlns="http://www.w3.org/2000/svg" xlink="http://www.w3.org/1999/xlink" width="30px" height="30px" viewBox="0 0 60 60" space="preserve">
										<desc>Telegram</desc>
										<path
											style="fill:#ffffff;"
											d="M30 0C13.4 0 0 13.4 0 30s13.4 30 30 30 30-13.4 30-30S46.6 0 30 0zm16.9 13.9l-6.7 31.5c-.1.6-.8.9-1.4.6l-10.3-6.9-5.5 5.2c-.5.4-1.2.2-1.4-.4L18 32.7l-9.5-3.9c-.7-.3-.7-1.5 0-1.8l37.1-14.1c.7-.2 1.4.3 1.3 1z"
										></path>
										<path style="fill:#ffffff;" d="M22.7 40.6l.6-5.8 16.8-16.3-20.2 13.3"></path>
									</svg>
								</a>
								<a href="whatsapp://send?phone=78432126177">
									<svg
										version="1.1"
										id="Layer_1"
										xmlns="http://www.w3.org/2000/svg"
										xlink="http://www.w3.org/1999/xlink"
										x="0px"
										y="0px"
										width="30px"
										height="30px"
										viewBox="0 0 99 99"
										style="enable-background:new 0 0 99 99;"
										space="preserve"
									>
										<path
											style="fill:#ffffff;"
											d="M63.892 53.114C63.143 52.739 59.462 50.928 58.775 50.678C58.089 50.428 57.589 50.303 57.09 51.053C56.591 51.803 55.156 53.489 54.719 53.989C54.282 54.489 53.845 54.551 53.097 54.176C52.348 53.801 49.935 53.011 47.075 50.459C44.849 48.473 43.346 46.021 42.909 45.272C42.472 44.522 42.862 44.117 43.237 43.744C43.574 43.408 43.986 42.869 44.36 42.432C44.734 41.995 44.859 41.682 45.109 41.183C45.359 40.683 45.234 40.246 45.047 39.871C44.86 39.496 43.362 35.81 42.738 34.311C42.13 32.851 41.513 33.049 41.053 33.026C40.617 33.004 40.117 33 39.618 33C39.119 33 38.308 33.187 37.621 33.937C36.935 34.687 35 36.498 35 40.184C35 43.87 37.683 47.431 38.058 47.93C38.432 48.43 43.338 55.993 50.85 59.237C52.637 60.009 54.031 60.469 55.119 60.814C56.913 61.384 58.545 61.304 59.836 61.111C61.275 60.896 64.266 59.3 64.89 57.551C65.514 55.802 65.514 54.302 65.327 53.99C65.14 53.677 64.641 53.489 63.892 53.114Z"
										></path>
										<path
											style="fill:#ffffff;"
											fill-rule="evenodd"
											clip-rule="evenodd"
											d="M49 98C76.062 98 98 76.062 98 49C98 21.938 76.062 0 49 0C21.938 0 0 21.938 0 49C0 76.062 21.938 98 49 98ZM50.221 17C58.238 17.003 65.763 20.124 71.421 25.789C77.08 31.454 80.194 38.983 80.189 46.992C80.182 63.515 66.737 76.96 50.22 76.96H50.208C45.193 76.958 40.264 75.7 35.887 73.312L20 77.479L24.252 61.949C21.63 57.405 20.25 52.249 20.252 46.967C20.258 30.443 33.703 17 50.221 17Z"
										></path>
									</svg>
								</a>
							</div>
						</div>
					</div>
				</ul>
			</div>
		</div>
	</div>
</div>

  <!-- Button trigger modal -->
  <div class="container mt-4 pb-4 d-none d-sm-block" style="z-index: 1000 !important; color: #fff;">
    <!-- Button trigger modal -->
      <span style="    color: #ffffff; font-size: 45px; font-family: 'Montserrat', Arial, sans-serif; line-height: 1.55;font-weight: 400;">
      <strong>Современный и уютный<br />
      дом в Казани и республике Татарстан<br />
      по цене 20 000₽ за м2</strong>
    </span>
    <br />
    <br />
    <span style="    color: #ffffff;font-size: 20px;font-family: 'Montserrat', Arial, sans-serif;line-height: 1.55; font-weight: 400; background-position: center center; border-color: transparent;border-style: solid;">
      По индивидуальному проекту<br />
      и по нашим типовым проектам
    </span>

      <br />
      <br /> 
      
      <a href="#rec448333531"><button
        class="p-4 mt-4"
        style="    color: #ffffff;font-size: 18px;font-family: 'Montserrat', Arial, sans-serif;line-height: 1.55;font-weight: 400;border-radius: 100px;background-color: #8ed834;background-position: center center; border-color: transparent; border-style: solid;
    transition: background-color 0.2s ease-in-out, color 0.2s ease-in-out, border-color 0.2s ease-in-out;"
      >
        <span style=" padding-right: 40px; padding-left: 40px;">Расчитать стоимость</span>
      </button></a>

      <div class="row row-cols-sm-1 row-cols-md-2 row-cols-lg-3 mt-5 pb-4 g-3">
        <div class="col">
          <div class="card-first p-4" style="    border-radius: 20px;background-color: #ffffff;background-position: center center;border-color: transparent;border-style: solid;">
            <div class="row">
              <div class="col-auto">
                <img class="mt-2" style="max-width:45px" src="https://static.tildacdn.info/tild6363-3630-4632-b963-616439313630/bi_check.svg" alt="" />
              </div>
              <a class="col" href="#rec448333531" style="text-decoration: none !important;">
                <span
                  style="    color: #000000;font-size: 18px;font-family: 'Montserrat', Arial, sans-serif;line-height: 1.55;font-weight: 400;background-position: center center;border-color: transparent;border-style: solid;
                "
                >
                  Загородные <br />
                  частные дома
                </span>
              </a>
            </div>
          </div>
        </div>

        <div class="col">
          <div class="card-first p-4" style="    border-radius: 20px;background-color: #ffffff;background-position: center center;border-color: transparent;border-style: solid;">
            <div class="row">
              <div class="col-auto">
                <img class="mt-2" style="max-width:45px" src="https://static.tildacdn.info/tild6363-3630-4632-b963-616439313630/bi_check.svg" alt="" />
              </div>
              <a class="col" href="#rec448333531" style="text-decoration: none !important;">
                <span
                  style="    color: #000000;font-size: 18px;font-family: 'Montserrat', Arial, sans-serif;line-height: 1.55;font-weight: 400;background-position: center center;border-color: transparent;border-style: solid;
                "
                >
                  Бани, гаражи и <br />
                  хозпостройки
                </span>
              </a>
            </div>
          </div>
        </div>

        <div class="col">
          <div class="card-first p-4" style="    border-radius: 20px;background-color: #ffffff;background-position: center center;border-color: transparent;border-style: solid;">
            <div class="row">
              <div class="col-auto">
                <img class="mt-2" style="max-width:45px" src="https://static.tildacdn.info/tild6363-3630-4632-b963-616439313630/bi_check.svg" alt="" />
              </div>
              <a class="col" href="#rec448333531" style="text-decoration: none !important;">
                <span
                  style="    color: #000000;font-size: 18px;font-family: 'Montserrat', Arial, sans-serif;line-height: 1.55;font-weight: 400;background-position: center center;border-color: transparent;border-style: solid;
                "
                >
                  Коммерческая <br />
                  недвижимость
                </span>
              </a>
            </div>
          </div>
        </div>
      </div>
  </div>

  <!-- mob -->
  <div class="container mt-4 pb-4 d-block d-sm-none" style="z-index: 1000 !important; color: #fff;">
    <span style="    color: #ffffff; font-size: 27px; font-family: 'Montserrat', Arial, sans-serif; line-height: 1.55;font-weight: 400;">
      <strong>Современный и уютный<br />
      дом в Казани и республике Татарстан<br />
      по цене 20 000₽ за м2</strong>
    </span>
    <br />
    <br />
    <span style="    color: #ffffff;font-size: 15px;font-family: 'Montserrat', Arial, sans-serif;line-height: 1.55; font-weight: 400; background-position: center center; border-color: transparent;border-style: solid;">
      По индивидуальному проекту<br />
      и по нашим типовым проектам
    </span>

    <br />

    <a href="#rec448333531"><button
      class="p-3 mt-4 w-100"
      style="       color: white;
    background: #8ED834 !important;
    border-radius: 67px !important;
    font-size: 15px;
    border: none !important;"
    >
      Расчитать стоимость
    </button></a>

    <div class="row row-cols-1 row-cols-sm-1 row-cols-md-2 row-cols-lg-3 mt-3 pb-4 g-3">
      <div class="col">
        <div class="card-first p-2" style="    border-radius: 20px;background-color: #ffffff;background-position: center center;border-color: transparent;border-style: solid;">
          <div class="row g-1">
            <div class="col-auto">
              <img class="p-3" style="max-width:45px" src="bi_check.svg" alt="" />
            </div>
            <div class="col pt-2">
              <span
                class="  "
                style="  vertical-align: middle ;   color: #000000;font-size: 15px;font-family: 'Montserrat', Arial, sans-serif;line-height: 1.55;font-weight: 400;background-position: center center;border-color: transparent;border-style: solid;
                "
              >
                Загородные частные дома
              </span>
            </div>
          </div>
        </div>
      </div>

      <div class="col">
        <div class="card-first p-2" style="    border-radius: 20px;background-color: #ffffff;background-position: center center;border-color: transparent;border-style: solid;">
          <div class="row g-1">
            <div class="col-auto">
              <img class="p-3" style="max-width:45px" src="bi_check.svg" alt="" />
            </div>
            <div class="col pt-2">
              <span
                style="    color: #000000;font-size: 15px;font-family: 'Montserrat', Arial, sans-serif;line-height: 1.55;font-weight: 400;background-position: center center;border-color: transparent;border-style: solid;
                "
              >
                Бани, гаражи и хозпостройки
              </span>
            </div>
          </div>
        </div>
      </div>

      <div class="col">
        <div class="card-first p-2" style="    border-radius: 20px;background-color: #ffffff;background-position: center center;border-color: transparent;border-style: solid;">
          <div class="row g-1">
            <div class="col-auto">
              <img class="p-3" style="max-width:45px" src="bi_check.svg" alt="" />
            </div>
            <div class="col pt-2">
              <span
                class=""
                style="    color: #000000;font-size: 15px;font-family: 'Montserrat', Arial, sans-serif;line-height: 1.55;font-weight: 400;background-position: center center;border-color: transparent;border-style: solid;
                "
              >
                Коммерческая недвижимость
              </span>
            </div>
          </div>
        </div>
      </div>
    </div>
  </div>
  <!-- Modal -->
  <?php 
  include 'menu_mobile.php'; 
  ?>
</div>
